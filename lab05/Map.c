...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	int i = 0,tmp = 0;
	for (VList s = g->connections[start]; s != NULL; s = s->next) {
	    if (s->v == end) {
			type[i] = s->type;
	        i++;
			if (s->type == BOAT) {
		...
		else if (s->type == BOAT && tmp == 0) {
			for (VList curr_sea = g->connections[s->v]; curr_sea != NULL; curr_sea = curr_sea->next) {
				if (curr_sea->v == end) {
					type[i] = curr_sea->type;
					i++;
					tmp = 1;
					break;
				}
